// backend/internal/db/GET_Ordenes_repo.go
package db
...
func NewOrdenesRepo(db *sql.DB) *OrdenesRepo {
	return &OrdenesRepo{DB: db}
}

func (r *OrdenesRepo) GetOrdenes() ([]OrdenResumen, error) {
	query := `
		SELECT
			o.id_order,
			o.numero_order,
			...
			SUM(d.cantidad * i.precio_unitario) AS total_general
		FROM "order" o
		...
		GROUP BY o.id_order, o.numero_order, o.fecha_expedicion, cl.id_contacto, 
			cl.razon_social, cl.ruc_ci, cp.describe_condicion
		ORDER BY o.fecha_expedicion DESC`

	rows, err := r.DB.Query(query)
	if err != nil {
		return nil, fmt.Errorf("error consultando órdenes: %w", err)
	}
	defer rows.Close()

	var ordenes []OrdenResumen
	for rows.Next() {
		var o OrdenResumen
		err := rows.Scan(
			&o.IDOrder, &o.NumeroOrder, &o.FechaExpedicion, &o.IDContacto,
			&o.RazonSocial, &o.RucCI, &o.DescribeCondicion, &o.TotalGeneral,
		)
		if err != nil {
...
// // backend/internal/db/GET_Historial_repo.go
// package db
